...
UserModel.register = async (user) => {
  try {
    ...
    if (reg) {
      const errorData = { status: 409, message: 'Email already registered' }
      return new Error(JSON.stringify(errorData))
    }

    reg = await User.findOne({ username: user.username })
    if (reg) {
      const errorData = { status: 409, message: 'Username already registered' }
      return new Error(JSON.stringify(errorData))
    }
    ...
    return new Error({ status: 500, message: 'Internal Server Error' })
  }
}
...
UserModel.getUser = async (id) => {
  try {
    const user = await User.findOne({ id })
    if (!user) {
      const errorData = { status: 404, message: 'User not found' }
      return new Error(JSON.stringify(errorData))
    }
    return user
  } catch (err) {
...
UserModel.rateSnippet = async (userId, snippetId) => {
  try {
    const user = await UserModel.getUser(userId)
    user.ratedSnippets.push(snippetId)
    await user.save()
    return user
  } catch (err) {
    const errorData = { status: 500, message: 'Internal Server Error' }
...
UserModel.deleteSnippetRatingsFromUsers = async (snippetId) => {
  try {
    const users = await User.find()
    for (const user of users) {
      const index = user.ratedSnippets.indexOf(snippetId)
      if (index > -1) {
        user.ratedSnippets.splice(index, 1)
      }
      await user.save()
    }
    return true
